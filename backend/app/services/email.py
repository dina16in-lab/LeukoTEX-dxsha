...
def send_inquiry_notification(inquiry: Any) -> Dict[str, Any]:
    """
    Send a LEUKOTEX project inquiry email through Resend.

    This version intentionally does NOT import any SQLAlchemy/database
    models, so the public contact API can work without PostgreSQL.

    The customer's email is used as Reply-To so LEUKOTEX can reply
    directly to the person who submitted the inquiry.
    """
    ...
    if not to_email:
        raise EmailDeliveryError(
            "EMAIL_TO is not configured on the server."
        )
    ...
    try:
        with httpx.Client(timeout=15.0) as client:
            ...
            if response.status_code >= 400:
                ...
                raise EmailDeliveryError(
                    f"Resend email delivery failed with status "
        ...
        logger.error(
            "Network exception while communicating with email service: %s",
            str(exc),
        )

        raise EmailDeliveryError(
            f"Network error while connecting to email service: {exc}"
        ) from exc