    ...
    <section className="w-full px-margin-mobile md:px-margin-desktop py-section-gap flex flex-col gap-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <span className="font-label-caps text-label-caps text-[#3E2723] uppercase relative z-10">
            Technologies
          </span>
          <ul className="flex flex-col gap-3 relative z-10 mt-6">
            {TECH_STACK.map((tech, i) => (
              <li
                key={tech.name}
                className={`font-label-mono text-[13px] text-[#3E2723] flex items-center justify-between ${
                  i !== TECH_STACK.length - 1 ? 'border-b border-border-metallic/60 pb-2.5' : ''
                }`}
              >
                <span>{tech.name}</span>
                <span className="material-symbols-outlined text-[16px] text-[#3E2723] group-hover:text-[#3E2723] transition-colors">
                  {tech.icon}
                </span>
              </li>
            ))}
        ...
        >
          ...
          <span className="relative z-10 font-headline-display text-7xl font-extrabold text-[#3E2723] opacity-60 group-hover:opacity-100 transition-opacity duration-500 tracking-tighter">
            LTX
          </span>
        </motion.div>
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 relative z-10 mt-6">
            {DISCIPLINES.map((d) => (
              <span
                key={d}
                className="px-3.5 py-1.5 bg-background/80 border border-border-metallic text-[#3E2723] font-label-mono text-[11px] uppercase rounded-full hover:border-secondary transition-colors"
              >
                {d}
              </span>
            ))}
        ...
        <motion.div
          ...
          className="col-span-1 md:col-span-2 lg:col-span-3 bg-primary text-[#3E2723] p-8 md:p-14 flex flex-col justify-center items-center text-center rounded-xl hover:shadow-[0_0_50px_rgba(255,255,255,0.12)] transition-shadow duration-700 group"
        >
          <span className="material-symbols-outlined text-[36px] text-[#3E2723]-fixed-variant mb-4 opacity-60">
            format_quote
          </span>
          <p className="font-headline-display text-2xl sm:text-3xl md:text-4xl leading-snug max-w-4xl font-normal text-[#3E2723]">
